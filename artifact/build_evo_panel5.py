...
(function () {
  ...
  function drawBeat4() {
    ...
    q.forEach(function (d, i) {
      ...
      // the last bar is the dip beat 4 is about — mark it, don't hide it
      var isDip = (i === n - 1);
      ctx.fillStyle = isDip ? meta : gold;
      ctx.globalAlpha = isDip ? 0.85 : 0.55;
      ctx.fillRect(x, y, bw, h);
      ctx.globalAlpha = 1;
      ctx.strokeStyle = isDip ? meta : gold; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(x, y + 0.75); ctx.lineTo(x + bw, y + 0.75); ctx.stroke();
